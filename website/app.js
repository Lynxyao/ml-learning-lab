...
function runGuidedWfmTraining({ epochs, useGan, useL1, noCheckpoints }) {
  ...
  liveLog.textContent = "";
  meter.style.width = "0";
  copy.textContent = `Loading the actual epoch ${epochs} WFM checkpoint...`;
...
function updateWfmTest() {
  ...
  const checkpoint = labState.wfm.epochs;
  ...
  if (labState.wfm.realtime && labState.wfm.testImageUrl) {
    ...
    image.alt = `WFM generated result comparison at epoch ${checkpoint}`;
    caption.textContent = `This is the actual saved visual checkpoint for epoch ${checkpoint}.`;
  }
  summary.innerHTML = `
...
function selectedEcgEpoch() {
  return ecgCheckpointEpochs[Number(ecgEpochs.value)];
}
ecgEpochs.addEventListener("input", () => {
  document.querySelector("#ecg-epoch-label").textContent = `Epoch ${selectedEcgEpoch()}`;
});
...
function runGuidedEcgTraining({ model, epochs, weighting }) {
  ...
  liveLog.textContent = "";
  meter.style.width = "0";
  copy.textContent = `Loading the real epoch ${epochs} teaching checkpoint...`;
  ...
  window.setTimeout(async () => {
    try {
      const response = await fetch(`assets/module2/checkpoint-${epochs}.json`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const metrics = await response.json();
      meter.style.width = "100%";
      copy.textContent = `Real epoch ${epochs} checkpoint loaded. Compare its held-out accuracy, macro-F1, and confusion matrix.`;
      labState.ecg = { model, epochs, weighting, metrics, savedCheckpoint: true };
      updateEcgTest();
    } catch (error) {
      meter.style.width = "0";
      copy.textContent = `Could not load the epoch ${epochs} checkpoint (${error.message}).`;
    }
  }, 1550);
}
...
function updateEcgTest() {
  ...
  pill.textContent = `${labState.ecg.model.toUpperCase()} · epoch ${labState.ecg.epochs}`;
  pill.classList.add("ready");
  if (metrics) {
    metricContent.accuracy.value = metrics.accuracy.toFixed(3);
    metricContent.accuracy.copy = labState.ecg.realtime
      ? `This is the real test accuracy from run ${labState.ecg.runId}.`
      : `This is the held-out accuracy saved at epoch ${labState.ecg.epochs}.`;
    metricContent.macro.value = metrics.macro_f1.toFixed(3);
    metricContent.macro.copy = labState.ecg.realtime
      ? `This is the real macro-F1 from run ${labState.ecg.runId}. Compare it with accuracy before judging the model.`
      : `This is the held-out macro-F1 saved at epoch ${labState.ecg.epochs}. Compare it with accuracy before judging the model.`;
    ...
    updateEcgConfusionMatrix(metrics.confusion_matrix, metrics.label_names);
  }
  summary.innerHTML = labState.ecg.realtime
    ...
    `
    : `
      <strong>Real saved teaching checkpoint</strong>
      <p>Model: 1D CNN. Epoch: ${labState.ecg.epochs}. Class weighting: on. Dataset: synthetic ECG teaching data with one fixed held-out split. Change only the epoch slider to make a fair checkpoint comparison.</p>
    `;
}

function updateEcgConfusionMatrix(matrix, labels) {
  ...
  document.querySelectorAll("[data-cell]").forEach((button) => {
    ...
    const row = labels.indexOf(truth);
    const col = labels.indexOf(prediction);
    if (row >= 0 && col >= 0) button.textContent = String(matrix[row][col]);
  });
}
...
function updateResistanceModelLogic() {
  ...
  logic.innerHTML = `
    ...
    <p>${model.logic}</p>
  `;
  loadResistanceHistory(modelKey);
}
...
async function loadResistanceHistory(modelKey) {
  const copy = document.querySelector("#resistance-history-copy");
  try {
    const response = await fetch(`assets/module4/history-${modelKey}.json`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const history = await response.json();
    const config =
      modelKey === "mlp"
        ? { key: "val_loss", label: "Validation loss" }
        : { key: "val_log_rmse", label: "Validation log-R RMSE" };
    drawResistanceHistory(history, config);
    const best = history.reduce((current, row) =>
      Number(row[config.key]) < Number(current[config.key]) ? row : current
    );
    copy.textContent = `${config.label} across ${history.length} real epochs. Lowest saved value: ${Number(
      best[config.key]
    ).toFixed(3)} at epoch ${best.epoch}. The reconstruction image below still comes from the selected final/best checkpoint.`;
  } catch (error) {
    copy.textContent = `Training history unavailable (${error.message}).`;
  }
}

function drawResistanceHistory(history, config) {
  const canvas = document.querySelector("#resistance-history-chart");
  ...
  const values = history.map((row) => Number(row[config.key]));
  ...
  context.strokeStyle = "#16705a";
  context.lineWidth = 2.5;
  context.beginPath();
  history.forEach((row, index) => {
    const x = padding.left + (index / (history.length - 1)) * chartWidth;
    const y = padding.top + ((maximum - Number(row[config.key])) / span) * chartHeight;
  ...
  context.fillText(maximum.toFixed(2), padding.left - 6, padding.top + 4);
  context.fillText(minimum.toFixed(2), padding.left - 6, padding.top + chartHeight + 4);
  context.textAlign = "left";
  context.fillText("1", padding.left, height - 8);
  context.textAlign = "right";
  context.fillText(String(history[history.length - 1].epoch), width - padding.right, height - 8);
  context.textAlign = "center";
  context.fillText("Epoch", padding.left + chartWidth / 2, height - 8);
}
